...
int main(int argc, char **argv)
{
  ...
  // SddLiteral sl = sdd_manager_var_count(manager);
  // printf("sdd manager var count:%d\n", sl);
  ...
  SddNode **array = sdd_node_elements(alpha);
  SddNodeSize size = sdd_node_size(alpha);
  char filename[50];
  int i = 0;
  for (SddNode **ele = array; ele < array + size * 2; ele++)
  {
    sprintf(filename, "output/sdd%d.dot", ++i);
    sdd_save_as_dot(filename, *ele);
  }
