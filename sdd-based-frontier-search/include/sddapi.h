...
typedef struct vtree_t* SddVtreeSearchFunc(struct vtree_t*,
...
SddManager* sdd_manager_copy(SddSize size, SddNode** nodes,
                             SddManager* from_manager);
...
void sdd_manager_set_minimize_function(SddVtreeSearchFunc func,
                                       SddManager* manager);
...
Vtree* sdd_manager_vtree_of_var(const SddLiteral var,
                                const SddManager* manager);
Vtree* sdd_manager_lca_of_literals(int count, SddLiteral* literals,
                                   SddManager* manager);
...
SddNode* sdd_apply(SddNode* node1, SddNode* node2, BoolOp op,
                   SddManager* manager);
...
SddNode* sdd_exists_multiple(int* exists_map, SddNode* node,
                             SddManager* manager);
SddNode* sdd_exists_multiple_static(int* exists_map, SddNode* node,
                                    SddManager* manager);
...
int sdd_manager_garbage_collect_if(float dead_node_threshold,
                                   SddManager* manager);
...
void sdd_manager_set_vtree_search_time_limit(float time_limit,
                                             SddManager* manager);
void sdd_manager_set_vtree_fragment_time_limit(float time_limit,
                                               SddManager* manager);
void sdd_manager_set_vtree_operation_time_limit(float time_limit,
                                                SddManager* manager);
void sdd_manager_set_vtree_apply_time_limit(float time_limit,
                                            SddManager* manager);
void sdd_manager_set_vtree_operation_memory_limit(float memory_limit,
                                                  SddManager* manager);
void sdd_manager_set_vtree_operation_size_limit(float size_limit,
                                                SddManager* manager);
...
SddWmc wmc_literal_weight(const SddLiteral literal,
                          const WmcManager* wmc_manager);
SddWmc wmc_literal_derivative(const SddLiteral literal,
                              const WmcManager* wmc_manager);
...
void sdd_vtree_var_order(SddLiteral* var_order, Vtree* vtree);
Vtree* sdd_vtree_first(const Vtree* vtree);
Vtree* sdd_vtree_last(const Vtree* vtree);
#endif  // SDDAPI_H_
